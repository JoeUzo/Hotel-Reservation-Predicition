...
class DataProcessor:
    ...
    def select_features(self, df):
        try:
            ...
            num = self.config['data_processing']['no_of_features']
            top_features = df[feature_importance_df['feature'][:num].values.tolist() + ['booking_status']]

            logger.info("Selected the top 10 important features")
            logger.info(f"Features selected:\n{top_features}")
    ...
    def save_data(self, df, file_path):
        try:
            logger.info("Saving our data in processed directory")
